  ...
  try {
    if (Array.isArray(texts)) {
      ...
      const translations = await Promise.all(
        texts.map((t) =>
          t.trim() ? translateSentence(t) : Promise.resolve(""),
        ),
      );
    ...
    logger.error("report-integration POST /translate", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
...
async function resolveInstitutionId(pool, userId, override) {
  ...
  const { rows } = await pool.query(
    `SELECT institution_id FROM public.users WHERE id = $1`,
    [userId],
  );
...
function resolveFieldLabel(field, language) {
  ...
  if (lbl && typeof lbl === "object")
    return lbl[language] || lbl.en || field.column_name;
...
function buildSelectClause(safeSelected, schemaFields, language = "en") {
  ...
  const selectParts = safeSelected.map((col) => {
    ...
    if (col === "created_at")
      return `TO_CHAR(r.created_at, 'DD Mon YYYY HH24:MI') AS "Submitted At"`;
...
function buildFullQuery(
  ...
  const { hasDept, hasUser, selectParts } = buildSelectClause(
    safeSelected,
    schemaFields,
    language,
  );
  ...
  const langClause =
    language === "hi"
      ? `r.language = 'hi'`
      : `(r.language = 'en' OR r.language IS NULL)`;
...
async function validateColumns(pool, tableName, requested) {
  ...
  return requested.filter(
    (c) => validSet.has(c) || c === "department_id" || c === "created_by",
  );
}
...
router.get("/forms", async (req, res) => {
  ...
  try {
    const iid = await resolveInstitutionId(
      pool,
      req.user.userId,
    ...
    if (!iid)
      return res
        .status(400)
        .json({ success: false, message: "institution_id required" });
    ...
    logger.error("report-integration GET /forms", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to list forms" });
  }
});
...
router.get("/forms/:formName/columns", async (req, res) => {
  ...
  try {
    ...
    if (!isValidRecordsTable(tableName))
      return res
        .status(400)
        .json({ success: false, message: "Invalid form name" });
    if (!(await tableExists(pool, tableName)))
      return res
        .status(404)
        .json({ success: false, message: "Form data table not found" });

    const dbCols = await getDbColumns(pool, tableName);

    // System columns that actually exist in this table
    const systemCols = SYSTEM_COLS.filter(
      (c) => dbCols.has(c.key) && !EXCLUDED_COLS.has(c.key),
    );
    ...
    let dynamicCols = schemaFields
      .filter(
        (f) =>
          f.column_name &&
          dbCols.has(f.column_name) &&
          !EXCLUDED_COLS.has(f.column_name),
      )
      .map((f) => ({
        key: f.column_name,
        label: resolveFieldLabel(f, language),
      }));
    ...
    logger.error("report-integration GET /forms/:formName/columns", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to get columns" });
  }
});
  ...
  try {
    ...
    if (await tableExists(pool, "custom_field_schemas")) {
      ...
      let sq = `SELECT schema -> 'fields' AS fields FROM public.custom_field_schemas
                WHERE form_name=$1 AND institution_id=$2 AND is_active=TRUE`;
      if (year) {
        qParams.push(Number(year));
        sq += ` AND year=$${qParams.length}`;
      }
      sq += ` ORDER BY created_at DESC LIMIT 1`;
    ...
    const baseQuery = buildFullQuery(
      tableName,
      safeSelected,
      iid,
      year,
    ...
    return res.json({
      success: true,
      data: { rows: pvRes.rows, total: Number(cntRes.rows[0]?.cnt || 0) },
    });
  } catch (err) {
    logger.error("report-integration GET /forms/:formName/preview", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to preview: " + err.message });
  }
});
...
router.post("/sections/:sectionId/blocks/table-import", async (req, res) => {
  ...
  try {
    ...
    const orderIndex =
      req.body.orderIndex != null ? req.body.orderIndex : req.body.order_index;
    ...
    if (!formName || !Array.isArray(selectedColumns) || !selectedColumns.length)
      return res
        ...
        .json({
          success: false,
          message: "form_name and columns are required",
        });
    ...
    if (!iid)
      return res
        .status(400)
        .json({ success: false, message: "Could not determine institution" });
    ...
    const safeSelected = await validateColumns(
      pool,
      tableName,
      selectedColumns,
    );
    ...
    const storedQuery = buildFullQuery(
      tableName,
      safeSelected,
      iid,
      year,
    ...
    const columnMap = [
      { key: "__row_id", label: "Row ID", hidden: true },
      ...columnMeta,
    ];
    ...
    try {
      ...
      if (language === "en") {
        ...
        if (hiRows.length) {
          const hiColumnMeta = buildColumnMeta(
            safeSelected,
            schemaFields,
            "hi",
          );
    ...
    logger.error("report-integration POST table-import", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to import: " + err.message });
  }
});
...
router.post("/blocks/:blockId/refetch", async (req, res) => {
  ...
  try {
    ...
    if (block.block_type !== "TABLE")
      return res
        .status(422)
        .json({ success: false, message: "Not a TABLE block" });
    if (!block.ds_id || !block.ds_query)
      return res
        .status(422)
        .json({ success: false, message: "Block has no form data source" });
    ...
    await pool
      .query(
       ...
       WHERE id=$3`,
        [
          JSON.stringify({ imported_at: importedAt }),
          req.user.userId,
          block.ds_id,
        ],
      )
    ...
    const primaryLanguage =
      (block.content || {}).language === "hi" ? "hi" : "en";
    let translations = {};
    if (
      primaryLanguage === "en" &&
      block.ds_params?.form_name &&
      block.ds_params?.selected_columns?.length
    ) {
      try {
        const {
          form_name: formName,
        ...
        if (isValidRecordsTable(tableName)) {
          ...
          if (iid && safeSelected.length) {
            ...
            if (await tableExists(pool, "custom_field_schemas")) {
              ...
              sq += ` ORDER BY created_at DESC LIMIT 1`;
              const { rows } = await pool.query(sq, qParams);
              schemaFields = Array.isArray(rows[0]?.fields)
                ? rows[0].fields
                : [];
            }
            const hiQuery = buildFullQuery(
        ...
        logger.error("report-integration refetch: hi translation sync failed", {
          ...getLogContext(req),
          err: e.message,
        });
      }
    ...
    logger.error("report-integration POST /blocks/:blockId/refetch", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to re-fetch: " + err.message });
  }
});
...
router.post("/blocks/:blockId/switch-language", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      (block.content || {}).source !== "form_import"
    )
      return res
        .status(422)
        .json({ success: false, message: "Not a form-import TABLE block" });
    ...
    if (!formName || !selected.length)
      return res
        ...
        .json({
          success: false,
          message: "Data source is missing import parameters",
        });
    ...
    await pool
      .query(
       ...
       WHERE id=$4`,
        [
          newQuery,
          JSON.stringify({ language, imported_at: importedAt }),
          req.user.userId,
          block.ds_id,
        ],
      )
    ...
    logger.error("report-integration POST /blocks/:blockId/switch-language", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      ...
      .json({
        success: false,
        message: "Failed to switch language: " + err.message,
      });
  }
});
...
function extractKpiDataFromSvg(svg) {
  ...
  try {
    ...
    const midYs = sortedYs.filter(
      (y) => y > seriesY && (totalsY < 0 || y < totalsY),
    );
    ...
    return {
      columns,
      series: [{ name: seriesName, display_name: seriesName, values }],
      totals,
    };
  } catch {
    return { columns: [], series: [], totals: [] };
...
router.get("/kpi-reports/years", async (req, res) => {
  ...
  try {
    ...
    logger.error("report-integration GET /kpi-reports/years", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to list years" });
  }
});
...
router.get("/kpi-reports", async (req, res) => {
  ...
  try {
    ...
    logger.error("report-integration GET /kpi-reports", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to list KPI reports" });
  }
});
...
router.get("/kpi-reports/:id", async (req, res) => {
  ...
  try {
    if (!(await kpiTableExists(pool)))
      return res
        .status(404)
        .json({ success: false, message: "KPI reports table not found" });

    const kpiId = Number(req.params.id);
    if (!Number.isFinite(kpiId) || kpiId <= 0)
      return res
        .status(400)
        .json({ success: false, message: "Invalid KPI id" });
    ...
    logger.error("report-integration GET /kpi-reports/:id", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      .status(500)
      .json({ success: false, message: "Failed to load KPI" });
  }
});
...
router.post("/sections/:sectionId/blocks/kpi-import", async (req, res) => {
  ...
  try {
    ...
    const orderIndex =
      req.body.order_index != null ? Number(req.body.order_index) : null;
    const caption = req.body.caption || "";

    if (!Number.isFinite(kpiReportId) || kpiReportId <= 0)
      return res
        .status(400)
        .json({ success: false, message: "kpi_report_id required" });
    ...
    if (!kpiRows.length)
      return res
        .status(404)
        .json({ success: false, message: "KPI report not found" });
    ...
    logger.error("report-integration POST /kpi-import", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      ...
      .json({
        success: false,
        message: "Failed to import KPI: " + err.message,
      });
  }
});
...
router.post("/blocks/:blockId/kpi-reimport", async (req, res) => {
  ...
  try {
    ...
    const block = blkRows[0];
    if (
      block.block_type !== "KPI" ||
      (block.content || {}).source !== "kpi_import"
    )
      return res
        .status(422)
        .json({ success: false, message: "Not a KPI import block" });

    const kpiReportId = Number(block.content.kpi_report_id);
    if (!Number.isFinite(kpiReportId) || kpiReportId <= 0)
      return res
        .status(422)
        .json({ success: false, message: "Block has no kpi_report_id" });
    ...
    const { rows: updRows } = await pool.query(
      `SELECT content FROM public.section_blocks WHERE id=$1`,
      [blockId],
    );
    ...
    logger.error("report-integration POST /kpi-reimport", {
      ...getLogContext(req),
      err: err.message,
    });
    return res
      ...
      .json({
        success: false,
        message: "Failed to re-import KPI: " + err.message,
      });
  }
});
