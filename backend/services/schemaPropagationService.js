...
async function ensureSchemaExists(pool, formName) {
  ...
  const { rows: tl } = await pool.query(
    `SELECT COALESCE(institute_access, '{}') AS institute_access, share_table
...
module.exports = { ensureSchemaPropagationLog, ensureSchemaExists, propagateAllSharedSchemas };
