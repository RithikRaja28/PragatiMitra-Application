  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.28)", zIndex: 1000,
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", width: 340,
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: "#1e293b", marginBottom: 4 }}>{t("Reassign Task", lang)}</div>
...
export default function TaskOverviewPage() {
  ...
  const handleReassign = (id, newAssignee) => {
    setTasks(prev => prev.map(task => task.id === id ? { ...task, assigned: newAssignee, lastUpdate: "just now" } : task));
    setReassigning(null);
  ...
  const filtered = tasks.filter(task => {
    if (filters.search   && !task.section.toLowerCase().includes(filters.search.toLowerCase())) return false;
...
=======
      <PageHeader
        ...
        description="All section assignments, statuses and deadlines for Samhita Siddhanta"
        actions={
          overdueCount > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 14px",
              background: "#fff1f2", border: "0.5px solid #fecdd3", borderRadius: 9 }}>
...
>>>>>>> 2dfc12740a97cede23f7be06dd88218fc7713123
          ...
          <option value="Not Started">{t("Not Started", lang)}</option>
        </select>
      </div>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: "32px", textAlign: "center", fontSize: 13, color: C.textSub }}>
                {t("No tasks match the current filters.", lang)}
              </td></tr>
            ) : filtered.map((task, i) => (
              <tr key={task.id}
                ...
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{task.section}</span>
                  </div>
                </td>
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none",
                  fontSize: 12, color: C.text }}>{task.assigned}</td>
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none" }}>
                  <span style={{ fontSize: 10, fontWeight: 600, padding: "3px 9px", borderRadius: 20,
                    background: STATUS_STYLE[task.status]?.bg, color: STATUS_STYLE[task.status]?.color }}>
                    {t(task.status, lang)}
                  </span>
                </td>
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none",
                  fontSize: 12, fontWeight: task.overdue ? 600 : 400,
                  color: task.overdue ? "#dc2626" : C.textSub }}>{task.deadline}</td>
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none",
                  fontSize: 12, color: C.textSub }}>{task.lastUpdate}</td>
                <td style={{ padding: "13px 16px", borderTop: i > 0 ? `0.5px solid ${C.border}` : "none" }}>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => setReassigning(task)}
                    ...
                    {task.overdue && (
                      <button onClick={() => handleRemind(task)}
      ...
      <div style={{ display: "flex", gap: 20, marginTop: 16, padding: "12px 16px",
        ...
        <span>{t("Total:", lang)} <b style={{ color: C.text }}>{tasks.length}</b></span>
        {Object.entries(STATUS_STYLE).map(([s]) => {
          const count = tasks.filter(task => task.status === s).length;
          return count > 0 ? (
            <span key={s}>{t(s, lang)}: <b style={{ color: STATUS_STYLE[s].color }}>{count}</b></span>
          ) : null;
        })}
      </div>
    </div>
