...
import { PageContainer, Button, Badge, EmptyState, DataTable, Dropdown, MenuItem, MenuLabel, Pagination } from "../../../ui";
...
function UserForm({ mode, entity, onCreated, onSaved, onBack, apiFetch }) {
  ...
  useEffect(() => {
    if (!form.institution_id) { setDepartments([]); setInstitutionDomain(""); return; }
    ...
    apiFetch(`/api/lookup/institution-domain?institution_id=${form.institution_id}`)
      .then((r) => r.json())
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          type="email"
          autoComplete="off"
          placeholder={institutionDomain ? `e.g. arun@${institutionDomain}` : "e.g. arun@aiia.edu.in"}
          value={form.email}
      ...
      <div>
        ...
        <Select
          hasError={!!fieldErrs.institution_id}
