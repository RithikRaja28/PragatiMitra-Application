...
function Skel({ w = "100%", h = 14, r = 6, mb = 0 }) {
  return (
    <div style={{ width: w, height: h, borderRadius: r, background: "#f1f5f9", marginBottom: mb,
      animation: "pgSkPulse 1.4s ease-in-out infinite" }} />
  );
}

export default function PgStudentDashboardPage() {
  ...
  return (
    <PageContainer style={{ gap: 14 }}>
      ...
      <style>{`@keyframes pgSkPulse { 0%,100%{opacity:1} 50%{opacity:0.45} }`}</style>

      <PageHeader
        breadcrumb={[t("Home", lang), t("Dashboard", lang)]}
        title={t("My Workspace", lang)}
        description={user?.institutionName || t("PG Student", lang)}
      />
